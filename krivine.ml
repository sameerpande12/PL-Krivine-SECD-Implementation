...
type value  = NumVal of int | BoolVal of bool | TupVal of int * (value list) | VClos((string* expr) * table)

type stackElement = CLOSURE of closure | ADD of closure | MULT of closure | AND of closure | OR of closure | NOT | NEGATIVE | ABS| CMP | IFTE (closure * closure) | TUP | PROJ(int*int)
...
let rec runKrivine clos stack =  match clos withs
   Clos(V(x),gamma) ->begin
  ...
  | Clos(Bool(n),gamma) ->begin
    ...
    | NOT :: stack1 ->
      runKrivine CLOSURE(Clos(Bool(not n),gamma))::stack1
    ...
    | TUP(r_completed, current::[])::stack1 ->
      Tclos( List.rev (clos::r_completed))
    | IFTE(clos1,clos2)::stack1 ->
      if n then runKrivine clos1 stack1
  ...
  | Clos(Tuple(n,elist),gamma)->
      runKrivine Clos(List.hd elist,gamma)   (TUP( [] , elist))::stack (*TUP(l1,l2) the head of l2 is the main closure that is being evaluated now and l1 is list of evaluated closures in reverse order*)
...
let rec resolveClosure clos = match clos with
  ...
  | Clos(Lamda(V(x),e),gamma) -> VClos((x,e),gamma)
  | Tclos(clist)->
